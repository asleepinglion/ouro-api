...
module.exports = Ouro.Application.extend({
  ...
  beforeSetup: function() {
    ...
    //maintain alternate case insensitive list of controllers
    this._controllers = {};
  ...
  beforeBoot: function() {
    ...
    console.log(colors.red("                       Y                   Y     Version " + Ouro.version +  "\n"));

  },

  afterBoot: function() {
    this.loadAdapters();
    this.loadApis(this.paths.cwd + 'node_modules/ouro-api/apis');
    this.loadApis(this.paths.cwd + 'apis');
    this.loadModels();
  ...
  loadAdapters: function() {
    ...
    if( adapters ) {
      ...
      for( var adapter in adapters ) {
        ...
        if( fs.existsSync(this.paths.cwd + '/node_modules/ouro-' + adapter + '/lib/' + adapter + '/class.js' ) ) {
  ...
  loadAdapter: function(adapter, options) {
    ...
    //load the adapter
    var Adapter = require(this.paths.cwd + '/node_modules/ouro-' + adapter + '/lib/' + adapter + '/class' );
  ...
  loadController: function(controllerName, controllerPath) {
    ...
    if( fs.existsSync(controllerPath + '/class.js') ) {
      ...
      //keep alternate case insensitive reference
      this._controllers[changeCase.lower(changeCase.camel(controllerName))] = controller;
  ...
  loadActions: function(controller, controllerPath) {
    ...
    actions.map(function(actionName) {
      ...
      if( fs.lstatSync(actionsPath + '/' + actionName).isDirectory() ) {
        ...
        if( fs.existsSync(actionsPath + '/' + actionName + '/class.js') ) {
          ...
          //store lowercase lookup for action
          controller._actions[changeCase.lower(changeCase.camel(actionName))] = action;
    ...
    self.log.debug('loaded action:', Object.keys(controller._actions));
  ...
  routeRequest: function(resolve, reject, req) {
    ...
    if( path.length > 1 ) {
      ...
      var controllerName = changeCase.lower(changeCase.camel(path[1]));
      ...
      if( path.length == 2 ) {
        ...
        if(_.isEmpty(controllerName) ) {
        ...
        } else if( controllerName === 'describe' ) {
          ...
          if( controllerName in self.controllers ) {
            ...
            //set controller name
            req.controller = this._controllers[controllerName].name;
      ...
      //handle RPC routes
        ...
        var actionName = changeCase.lower(changeCase.camel(path[2]));
        ...
        if( controllerName in self._controllers ) {
          ...
          if( self.controllers[req.controller]._actions[actionName] ) {

            //set the action name
            req.action = self.controllers[req.controller]._actions[actionName].name;
      ...
      if( !req.controller ) {
        return reject(new Ouro.Error('controller_not_found', 'Controller not found.', {status: 404}));
      }

      //reject if the requested action doesn't exist
      if( !req.action ) {
        if( req.actionType === 'REST' ) {
          return reject(new Ouro.Error('action_not_found', 'REST Controller action ' + req.method + ' not implemented.', {status: 400}));
        } else {
          return reject(new Ouro.Error('action_not_found', 'Controller RPC action ' + req.method + ' not found.', {status: 404}));
        }
      }
      ...
      reject(new Ouro.Error('malformed_request', 'Something went wrong trying to process your request.', {status: 500}));
  ...
  authRequest: function(resolve, reject, req) {
    ...
    if( self.controllers[req.controller][req.action].meta && self.controllers[req.controller][req.action].meta.security === false ) {
      ...
      if( !self.controllers[controllerName] || !self.controllers[controllerName].authorize ) {
        ...
        return reject(new Ouro.Error('authorize_not_configured', "The " + controllerName + " controller's _authorize method has not been implemented.", {status: 500}));
      ...
      if(  self.config.mock.specific === false ||
        ...
        if (fs.existsSync(mockPath + '/' + mockFile + '.js')) {
          ...
          if (typeof mock !== 'object' ||
            ...
            return reject(new Ouro.Error('mock_invalid', 'The mock of the ' + mockFile + ' response for the ' + req.controller + '.' + req.action + ' action was invalid.', {status: 500}));
          }
          ...
          //let ouro know we already responded to the request.
          res.responded = true;
          ...
          if (self.config.mock.required !== true && !req.param('_mock') ) {
            ...
            reject(new Ouro.Error('mock_not_found', 'The mock of the ' + mockFile + ' response for the ' + req.controller + '.' + req.action + ' action was not found.', {status: 404}));
          }
        }
        ...
        if (self.config.mock.required !== true && !req.param('_mock') ) {
          ...
          reject(new Ouro.Error('mock_not_found', 'The mock of the ' + mockFile + ' response for the ' + req.controller + '.' + req.action + ' action was not enabled.', {status: 404}));
        }
      }
