...
module.exports = SuperJS.Application.extend({
    ...
    if( fs.existsSync(self.paths.cwd+'/apis') ) {
      ...
      apis.map(function(apiName) {
        ...
        if( fs.existsSync(self.paths.cwd+'/apis/'+apiName+'/model.js') ) {
          ...
          if( Model && typeof Model.prototype.adapter === 'string' ) {

            if( typeof self.adapters[Model.prototype.adapter] !== 'object' ) {

              self.log.warn('missing model adapter:', Model.prototype.adapter);

            } else {

              if( typeof self.resources.models[Model.prototype.adapter] !== 'object' ) {
                self.resources.models[Model.prototype.adapter] = {};
              }

              self.resources.models[Model.prototype.adapter][apiName] = Model;
  ...
  loadModels: function() {
    ...
    for( var adapter in self.resources.models ) {
      ...
      self.log.debug('loaded ' + adapter + ' modesls:', Object.keys(self.resources.models[adapter]));
