...
module.exports = SuperJS.Application.extend({
  ...
  loadActions: function(controller, controllerPath) {
    ...
    actions.map(function(actionName) {
      ...
      console.log(Object.keys(controller._actions));
    ...
    if( path.length > 1 ) {
      ...
      //handle RPC routes
        ...
        if( path[1] in self.controllers ) {
          ...
          console.log(Object.keys(self.controllers[path[1]]._actions));

          //check if the method exists on controller and its configured as an action
          if( self.controllers[path[1]]._actions[changeCase.lower(path[2])] ) {

            //set the action name
            req.action = self.controllers[path[1]]._actions[changeCase.lower(path[2])].name;
