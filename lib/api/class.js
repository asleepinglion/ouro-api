  ...
  loadControllers: function() {
    ...
    if( fs.existsSync(self.paths.cwd + '/apis') ) {
      ...
      modules.map(function(apiName) {
        ...
        if( self.config.enabled && self.config.enabled.indexOf(apiName) === -1 ) {
          return;
        }

        //make sure the controller exists
        if (fs.existsSync(self.paths.cwd + '/apis/' + apiName + '/controller/class.js')) {

          var Controller = require(self.paths.cwd + '/apis/' + apiName + '/controller/class');
      ...
      .then(function() {
        ...
        if( !res.responded ) {
          ...
          return self.controllers[req.controller].beforeAction(req)
            ...
            .then(function(response) {
              if( self.controllers[req.controller]['verified' + camelCaseName] ) {
                return self.controllers[req.controller]['verified' + camelCaseName](req);
              } else {
                return {};
              }
            })
